...
public class pill_box {
   ...
   public void loadAllDoses() {
    try {
        LocalDate today = LocalDate.now();
        String retrieveDoses = String.format("SELECT dose_time FROM schedule;");
        List<List<String>> doseTimes = executeQueryAndReturnResult(retrieveDoses);

        for (List<String> row : doseTimes) {
            String doseTime = row.get(0);

            String insertQuery = String.format("INSERT INTO adherence (date, dose_time, open_time, status) " + 
            "VALUES ('%s', '%s', NULL, 'Missed') ON CONFLICT(date, dose_time) DO NOTHING;", today, doseTime);
            executeUpdate(insertQuery);
        }
        String testQuery = String.format("SELECT date, dose_time, status FROM adherence ORDER BY dose_time;");
        executeQueryAndPrintResult(testQuery);
        System.out.println("Dose times have been loaded in!");
    }
    catch (Exception e) {
        System.err.println (e.getMessage ());
        return;
